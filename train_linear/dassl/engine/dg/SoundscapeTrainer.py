...
from dassl.engine.dg.SoundscapeStyler import SoundscapeStyler
...
class SoundscapeTrainer(TrainerBase):
    def __init__(self, cfg):
        ...
        weight_dir_path = cfg.TRAINER.SOUNDSCAPE.WEIGHT_DIR_PATH
        if not os.path.exists(weight_dir_path):
            os.makedirs(weight_dir_path)
        self.weight_save_path = os.path.join(weight_dir_path, cfg.TRAINER.SOUNDSCAPE.CHECK_POINT_NAME)
    ...
    def build_model(self):
        ...
        self.model = SoundscapeStyler(cfg, self.classnames, self.clip_model, self.device)
    ...
    def train(self):
        ...
        for style_idx in range(0, self.n_style):
            for epoch in range(self.max_epoch):
                ...
                # model forward
                style_diversity_loss, acoustic_loss = self.model(style_idx)
                # *********Total loss**************
                total_loss = style_diversity_loss + acoustic_loss
                total_loss.backward()
                self.optim.step()
                self.sched.step()
                ...
                if (epoch + 1) % 20 == 0 or epoch == 0:
                    ...
                    info += [f"acoustic_loss {acoustic_loss.item()}"]

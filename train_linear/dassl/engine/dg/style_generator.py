...
class BaseStyleGenerator(nn.Module):
    def __init__(self, cfg, classnames, clip_model, device):
        ...
        # 构建基础特征向量
        # 给每一个类别构建一个基础风格向量
        base_text_list = ["this is sound of " + s for s in classnames]
    ...
    def train_data(self):
        ...
        "style_prompt":n_style background
        '''
...
class SoundscapeGenerator(BaseStyleGenerator):
    def __init__(self, cfg, classnames, clip_model, device):
        super().__init__(cfg, classnames, clip_model, device)
        self.style_embedding = None  # to be optimized n_style*1*512
        self.weight_save_path = os.path.join(cfg.TRAINER.SoundscapeStyler.WEIGHT_DIR_PATH,
                                             cfg.TRAINER.SoundscapeStyler.CHECK_POINT_NAME)
        self.load_weight()

    def reinit_style(self, embedding_dim=512):
        print("soundscapeGenerator not need to call reinit_style")
